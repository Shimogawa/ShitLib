...
namespace ShitLib.Net.Bilibili.BLiveDanmaku
{
    public class BDanmakuGetter
    {
        ...
        public BDanmakuList DanmakuList { get; private set; }

        public BDanmakuGetter(int roomUrlID)
        {
            _roomUrlID = roomUrlID;
            DanmakuList = new BDanmakuList();
        }
        ...
        public void StartLoop()
        {
            while (true)
            {
                ...
                {
                    Console.WriteLine("Request time out. Trying again.");
                }
                catch (WebException)
                {
                    Console.WriteLine("Request time out. Trying again.");
...
//            _socket.Connect(_host, _port);
            ...
            DanmakuList.AddDanmaku(new BMessageInfo(BMessageType.Log, new BOtherMsg($"链接房间 {_roomUrlID} 成功")));
...
//                                Console.WriteLine($"Viewing: {viewing}");
                                if (viewing >= 3)
                                {
                                    DanmakuList.AddDanmaku(new BMessageInfo(BMessageType.OnlineViewerInfo, 
...
//            Console.WriteLine(ToHexString(sendbytes));
        ...
        private static BMessageInfo ParseDanmaku(byte[] data, int offset, int count)
        {
            var json = Encoding.UTF8.GetString(data, offset, count);
            var jobj = JObject.Parse(json);
            var cmd = jobj["cmd"].Value<string>();
            BMessageInfo info = null;
            if (cmd == "DANMU_MSG")
            {
                ...
                info = new BMessageInfo(BMessageType.Danmaku, new BDanmaku(prefix, user, danmaku));
            }
            else if (cmd == "SEND_GIFT")
            {
                ...
                info = new BMessageInfo(BMessageType.Gift, 
                    new BGift(user, giftName, num));
            }
            else if (cmd == "WELCOME")
            {
                var user = jobj["data"]["uname"].Value<string>();
                info = new BMessageInfo(BMessageType.EnterRoom, new BWelcome(user));
            }
